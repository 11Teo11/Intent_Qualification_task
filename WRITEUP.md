...
* **What signals does the system rely on most heavily and when are they misleading?** The system relies heavily on the dynamic weighting of Semantic vs. Lexical scores based on query length. This can be misleading if a user types a very long query that is actually just searching for a highly specific keyword, as the system will artificially lower the BM25 weight and might lose the exact match.
